...
router.get("/:id", (req, res) => {
  ...
  connection.query(
    `SELECT * FROM busdriver WHERE driverID= ${id}`,
...
router.patch("/:id", (req, res) => {
  ...
  console.log(id);
  connection.query(
    `UPDATE busdriver SET driverName = '${req.body.fullName}', driverPhoneNumber = '${req.body.phoneNumber}', driverAddress = '${req.body.address}'   WHERE driverID = ${id};`,
    function (error, results) {
      if (results) {
        res.status(200).json({
